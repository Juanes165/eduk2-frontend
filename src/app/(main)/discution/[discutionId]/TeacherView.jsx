...
export default function TeacherView({ params }) {
    const { user } = useAuth();

    const discutionId = params.discutionId

    const [ownComent, setOwnComent] = useState(true)
            ...
            <section className="mt-5 text-center md:px-40 2xl:px-96">
                ...
                <div className="mt-5 flex flex-col gap-3 text-start">
                    {discution?.listComments?.map((coment, index) => (
                        <ComentCard coment={coment} key={index} ownComent={ownComent} handleDelete={handleToggleModal}/>
                    ))}
                </div>
                {confirmDeleteModal &&
